  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#f3f4f5]">
        <p className="animate-pulse text-gray-400">A carregar sugestões...</p>
      </div>
    );
  }

  return <RecommendationsBody recommendations={data} />;
}
    ...
    return (
      <div className="min-h-screen bg-[#f3f4f5] flex flex-col items-center justify-center pb-20 text-center px-4">
        <h1 className="text-2xl font-normal text-gray-400 tracking-tight">
          Ainda não temos recomendações para ti.
        </h1>
        <p className="text-gray-400 mt-2">Adiciona alguns interesses para começarmos!</p>
      </div>
    );
  }
...
function CategoryCarousel({ title, items, creatorKey }: CarouselProps) {
  // Começa o índice no meio para o efeito visual inicial
  ...
  return (
    <div className="w-full">
        ...
        <div className="flex items-center justify-center gap-2 md:gap-4 lg:gap-6 h-[240px] w-full">
          {items.map((item, index) => (
            <div
              ...
              className={`${getStyles(index)} bg-[#d9d9d9] rounded-[4px] shrink-0 cursor-pointer overflow-hidden relative group`}
            >
              {/* Overlay suave para melhorar o aspeto enquanto não há imagem */}
      ...
      {activeItem && (
        <div className="text-center mt-6 h-20 animate-fadeIn">
