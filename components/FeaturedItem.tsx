...
export default function FeaturedItem({
  ...
  const renderImage = (
    ...
    const imgContent = (
      <div
        className={`relative z-10 ${sizeClass} ${aspectClass} rounded-app overflow-hidden border-2 border-app-accent bg-app-nav flex-shrink-0 transition-all duration-300 ${isOwner || isLink ? 'group-hover/shelf:scale-105 group-hover/shelf:border-app-font' : ''} ${opacityClass}`}
      >
        {imgUrl ? (
          <Image
            src={imgUrl}
      ...
      >
        ...
        {!items && (
          ...
          >
            ...
            {isPortrait && (
              <div
                className={`h-[50px] ${aspectClass} bg-app-accent opacity-[0.05] rounded-app flex-shrink-0`}
              />
            )}

            {/* Far Left - 65px */}
            <div
              className={`h-[65px] ${aspectClass} bg-app-accent opacity-20 rounded-app flex-shrink-0`}
            />
